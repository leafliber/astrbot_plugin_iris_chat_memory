...
本次更新聚焦记忆持久化与清理可靠性，增强索引恢复、模型迁移备份保护及 AstrBot 接口兼容性。

### 修复

- **图谱辅助检索返回失败**：`search_memory` 开启 `with_graph_context` 时正确拆分图谱格式化结果，避免将 `(文本, 节点 ID 集合)` 元组拼入字符串列表。
- **LLM 与异常路径**：传给 AstrBot 的历史字典消息转换为标准 `Message`，保持输入不被修改；Provider 获取仅接受聊天类型。数据库持锁操作补充关闭状态检查；Web L1 查询缺少会话参数、群画像更新收到空 JSON 时返回 400，并保留显式空会话键的访问兼容性。
- L2 FAISS 索引改为原子落盘，启动时对照 SQLite 补回缺失向量、移除残留向量；损坏索引保留为 `.corrupt` 后重建。新增隐藏参数 `l2_index_audit_interval_sec`（默认 600 秒，0 禁用周期对账），让启动恢复失败的条目可以在运行期间继续恢复。
- 索引恢复保留现有实际 Embedding 维度检测，补写前复查条目是否被删除或编辑；清理过期 free-list，逐批保存恢复进度，关闭时等待后台落盘任务。
- 模型迁移备份保存在插件数据目录的 `faiss/migration_backup/`；导出不完整时保留旧库，导入不完整或最终落盘失败时保留全量备份，仅在完整导入并保存成功后清理备份。
- 用户级 L2 清理兼容只有 `user_id` 的工具记忆；L2 与全量清理命令正确传递当前人格，修复非默认人格记忆漏删。指令解析兼容 AstrBot 的 `[At:ID]` 和 `@名字(ID)` 提及形式。
- L3 清理识别 `properties.user_id` 与跨群 `group_ids`，容忍损坏 JSON 并转义群 ID 中的 LIKE 通配符。工具保存知识及梦境提取补齐 Person 的稳定用户标记，昵称归一化后保留边引用。
- 隐藏配置持久化串行执行，失败后保留脏标志；加载时过滤错误类型，恢复默认值时正确通知观察者。

### 开发与质量

- **历史类型问题清理**：补齐懒加载导出的静态声明（保持运行时懒加载）、组件和配置类型、可空参数、任务返回值，以及 FAISS 动态绑定与 AstrBot 版本兼容接口的标注。
- 新增索引持久化与恢复、用户与人格范围清理、指令解析、隐藏配置容错、Web 请求校验及 LLM / 工具行为回归测试。
- 发布验证：全量测试 921 passed；`ruff check .` 通过；`pyright iris_memory/` 为 0 errors / 0 warnings。
