...
def _make_adapter() -> L2MemoryAdapter:
    ...
    mock_index.search = Mock(return_value=(np.array([[0.95]]), np.array([[0]])))
...
def _remaining_ids(adapter: L2MemoryAdapter) -> set:
    return {
        r[0] for r in adapter._db.execute("SELECT memory_id FROM memories").fetchall()
    }
...
class TestDeleteByUserScopes:
    ...
    async def test_delete_by_user_removes_tool_saved_memory(self):
        ...
        summary_mid = await _add(
            adapter,
            "总结记忆:用户养了一只猫",
            {"group_id": group_id, "source": "l1_summary", "active_users": user_id},
        )
    ...
    async def test_delete_by_user_hits_multi_user_scene_memory(self):
        ...
        await _add(
            adapter,
            "场景记忆:两人讨论了搬家",
            {"group_id": "g1", "source": "l1_summary", "active_users": "10001,10002"},
        )
    ...
    async def test_delete_by_user_no_substring_false_positive(self):
        ...
        await _add(
            adapter,
            "他人记忆",
            {"group_id": "g1", "source": "l1_summary", "active_users": "100011,10002"},
        )
